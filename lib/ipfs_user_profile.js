...
*/
    ...
    async add_profile(profile_data) {
        ...
        let save_biometric = storable_profile.biometric
        //
        delete storable_profile.public_key
        delete storable_profile.signer_public_key
        delete storable_profile.biometric
        //
        ...
        storable_profile.biometric = save_biometric
    ...
    async check_existence(storable_profile) {
        let storable = Object.assign({},storable_profile)
        delete storable.public_key
        delete storable.signer_public_key
        delete storable.biometric
        let btype = storable.business
        if ( typeof btype === 'boolean'  ) {
            btype = btype ? "business" : "profile"
        }
        //
        let clear_cid = await this.add_profile_file(storable)
        let path =  this.get_user_path(clear_cid,btype);
        let exists = await this.path_is_dir(path)
        return exists
    }
}
...
class IPFSProfiles extends IPFSProfilesPars {
    ...
    async add_profile_message_cid(body,subdir) {   // subdir is one of 'spool' or 'topic_spool'
        let message = body.message
        let receiver_cid = body.clear_cid
        let m_cid = await this.add_message_to_user_directory(receiver_cid,subdir,message)
        return(m_cid)
    }

...
class IPFSProfilesPars extends IPFSManageFiles {
    ...
    async add_contact_template_file(default_contact_page_path,form_link) {
        let fpath = `/${this.service_dir}/${this.template_input_sources}/`
        fpath += form_link.link
        ...
        if ( (this.from_file !== undefined) && this.from_file ) {   // a local directory
            data = (await fsPromise.readFile(this.from_file)).toString()

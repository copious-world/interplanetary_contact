...
class IPFSProfiles extends IPFSProfilesPars {
    ...
    async remove_messages(user_cid,btype,message_list) {
        ...
        u_path += '/spool/'
        for ( let mcid of message_list ) {
            await this.remove_cid(u_path,mcid)
        }
    }

    async move_messages_to_dir(user_cid,btype,parameter,message_list) {
        //
        if ( typeof btype === 'boolean'  ) {
            btype = btype ? "business" : "profile"
        }
        //
        let u_path = this.get_user_path(user_cid,btype)
        let from_path = u_path += '/spool/'
        let to_path = u_path += `/${parameter}/`

        for ( let mcid of message_list ) {
            await this.move_cid(from_path,to_path,mcid)
        }

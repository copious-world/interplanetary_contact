...
class IPFSManageFiles {
    ...
    // ---- ---- ---- ---- ---- ---- ---- ---- ---- ----
    async remove_file(u_path,f_name) {
        let ipfs = this.ipfs
        try {
            let fpath = `${u_path}/${f_name}`
            let file = await ipfs.files.stat(fpath)
            if ( file ) {
                await ipfs.files.rm(fpath)
            }
        } catch (e) {}
    ...
    async move_file(u_path,dst_path) {
        let ipfs = this.ipfs
        try {
            await ipfs.files.mv(u_path,dst_path,{"parents" : true})
        } catch (e) {
            console.dir(e)
        }
    }

    async path_is_dir(cat_path) {
        let ipfs = this.ipfs
        try {
            let cstat = await ipfs.files.stat(cat_path)
            let cid = cstat.cid.toString()
            if ( cid ) {
    ...
    async create_dir(dir_path) {
        let ipfs = this.ipfs
        try {
            await ipfs.files.mkdir(dir_path,{ 'parents' : true })
        } catch (e) {
            console.log(e)
        }
        try {
    ...
    async add_data(blob_holder) {
        let ipfs = this.ipfs
        let file = await ipfs.add(blob_holder)
